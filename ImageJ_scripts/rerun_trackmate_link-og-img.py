...
from fiji.plugin.trackmate.action import RecomputeFeatureAction
...
directory_to_process = os.path.normpath("E:/Nicholas/processed/trackmate_output/")
...
def run(xml_file):
  ...
  RecomputeFeatureAction.recompute(trackmate, logger)
  ...
  labelidpainting = lblImg.LabelIdPainting.LABEL_IS_SPOT_ID
  lblImg.createLabelImagePlus(trackmate, False, False, labelidpainting, logger).show()
  c = WindowManager.getCurrentImage()
  IJ.save(c, "E:/Nicholas/processed/label_images/" + c.getTitle())
  c.close()
